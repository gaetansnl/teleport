...
func (s *Server) startReconciler(ctx context.Context) error {
	...
	go func() {
		for {
			select {
			case <-s.reconcileCh:
				if err := reconciler.Reconcile(ctx, s.getReconcileDatabases().AsResources()); err != nil {
...
func (s *Server) getReconcileDatabases() types.Databases {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return append(append(s.cfg.Databases, s.dynamicDatabases...), s.cloudDatabases...)
}
